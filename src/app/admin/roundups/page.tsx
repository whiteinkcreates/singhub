  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-8 text-white">
      <section className="flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-300">
            Data first. This screen materializes the selected day from canonical SingHUB snapshots,
            validates it, and shows the exact rows that will eventually become the locked publishing artifact.
          </p>
        </div>
        ...
        <form className="flex items-end gap-3" method="get">
          <label className="grid gap-2 text-xs font-bold uppercase tracking-[0.16em] text-slate-300">
            Choose date
            <input
              className="rounded-xl border border-white/15 bg-slate-950 px-3 py-2 text-base font-semibold text-white"
              defaultValue={date}
              name="date"
              type="date"
            />
      ...
      <section className="mt-7 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-4">
          <p className="text-xs uppercase tracking-[0.16em] text-slate-400">Source snapshot</p>
          <p className="mt-1 text-sm font-bold text-slate-200">
            {draft.sourceLastSynced
              ? new Date(draft.sourceLastSynced).toLocaleString("en-US", { timeZone: "America/Los_Angeles" })
              : "Unknown"}
          </p>
        </div>
      </section>
      ...
      <section className="mt-7 rounded-3xl border border-white/10 bg-white/[0.03] p-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <h2 className="mt-1 text-2xl font-black">
              {blockers.length === 0 ? "Candidate can proceed to review" : "Candidate cannot be reviewed yet"}
            </h2>
          </div>
          <div className="rounded-full border border-white/10 px-3 py-1 text-xs font-black uppercase tracking-[0.15em] text-slate-300">
            DRAFT
          </div>
        </div>
        ...
        {draft.validation.length === 0 ? (
          <p className="mt-4 rounded-2xl border border-emerald-300/20 bg-emerald-300/10 p-4 text-sm text-emerald-100">
            No validation issues found in the loaded candidate.
          </p>
        ) : (
          <div className="mt-4 grid gap-3">
            {draft.validation.map((issue, index) => (
              <div
                className={`rounded-2xl border p-4 ${issueClasses(issue.severity)}`}
                key={`${issue.code}-${index}`}
              >
                <p className="text-xs font-black uppercase tracking-[0.15em]">
                  {issue.severity} · {issue.code.replaceAll("_", " ")}
                </p>
      ...
      <section className="mt-7 overflow-hidden rounded-3xl border border-white/10 bg-white/[0.03]">
        <div className="border-b border-white/10 p-5">
          ...
          <p className="mt-2 text-sm text-slate-400">
            Ordering and numbering shown here are candidate values. Review editing will happen before LOCKED.
            After locking, rendering will receive these strings directly and will not look them up again.
          </p>
        </div>
              ...
              {draft.rows.map((row) => (
                <tr className="border-t border-white/[0.06]" key={row.eventId}>
                  ...
                  <td className="px-4 py-3 text-slate-300">
                    {row.startTime || "Missing"}{row.endTime ? ` – ${row.endTime}` : ""}
                  </td>
