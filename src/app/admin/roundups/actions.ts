...
function groupsFor(rows: RoundupVenueRow[]) {
  ...
  for (let index = 0; index < rows.length; index += 5) {
    groups.push({ groupId: `venues-${String(groups.length + 1).padStart(2, "0")}`, venueEventIds: rows.slice(index, index + 5).map((row) => row.eventId) });
  }
  return groups;
}
...
export async function saveEditedRoundupDraftAction(formData: FormData) {
  const raw = String(formData.get("draft") || "");
  if (!raw) throw new Error("Missing edited roundup draft.");
  const submitted = JSON.parse(raw) as RoundupDraft;
  const normalized: RoundupDraft = {
    ...submitted,
    state: "draft",
    rows: submitted.rows.map((row, index) => ({ ...row, number: index + 1 })),
    groups: groupsFor(submitted.rows),
  };
  const validated = await revalidateRoundupDraft(normalized);
