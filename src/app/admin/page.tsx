...
const tools = [
  {
    ...
    title: "Daily Mic Generator",
    description:
      "Turn today’s poll into a branded feed or story card, generate stronger captions, and send people straight to /vote.",
  },
  {
    href: "/admin/scout",
    eyebrow: "Discovery",
  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-10 text-white">
      <section className="max-w-3xl">
        <p className="text-sm font-bold uppercase tracking-[0.3em] text-cyan-300">SingHUB Internal</p>
        <h1 className="mt-3 text-4xl font-black md:text-6xl">Admin Tools</h1>
        <p className="mt-4 text-slate-300">Backstage tools for keeping the Venue Index accurate, publishing content, and building venue partnerships.</p>
      </section>

      <section className="mt-8 grid gap-5 md:grid-cols-2">
        {tools.map((tool) => (
          <Link key={tool.href} href={tool.href} className="group rounded-[2rem] border border-white/10 bg-white/[0.04] p-6 transition hover:-translate-y-1 hover:border-cyan-300/40 hover:bg-cyan-300/[0.05]">
            <p className="text-xs font-black uppercase tracking-[0.22em] text-fuchsia-300">{tool.eyebrow}</p>
            <h2 className="mt-3 text-2xl font-black text-white group-hover:text-cyan-100">{tool.title}</h2>
