...
export function PollOfTheDay() {
  ...
  useEffect(() => {
    fetch("/api/polls/current", { cache: "no-store" })
      ...
      .catch(() => setMessage("Poll is taking the night off."))
  ...
  return (
    <section className="relative overflow-hidden rounded-[2rem] border border-fuchsia-300/20 bg-[linear-gradient(145deg,rgba(2,6,23,0.98),rgba(20,13,43,0.94)_52%,rgba(8,47,73,0.48))] shadow-[0_28px_80px_rgba(2,6,23,0.38)]">
      ...
      <div className="relative p-5 md:p-7">
        <div className="flex flex-col gap-4 border-b border-white/10 pb-5 md:flex-row md:items-center md:justify-between">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.25em] text-fuchsia-300">Real opinions. Real karaoke debate.</p>
            <h2 className="mt-2 text-2xl font-black tracking-tight text-white md:text-3xl">Poll of the Day</h2>
          </div>
          <div className="flex flex-wrap gap-2 text-xs font-bold text-slate-400">
            <span className="rounded-full border border-white/10 bg-white/[0.04] px-3 py-2">New poll tomorrow</span>
          ...
          <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_0.92fr]">
                ...
                {poll.options.map((option) => {
                  ...
                  return (
                    <button
                      key={option.id}
                      type="button"
                      disabled={Boolean(selected) || voting}
                      onClick={() => vote(option.id)}
                      className={`min-h-16 rounded-2xl border px-4 py-3 text-left text-sm font-black transition ${isSelected ? "border-fuchsia-300/70 bg-fuchsia-400/20 text-white shadow-[0_0_28px_rgba(217,70,239,0.16)]" : "border-white/12 bg-white/[0.035] text-slate-100 hover:-translate-y-0.5 hover:border-cyan-300/45 hover:bg-cyan-300/[0.06]"} disabled:cursor-default`}
                    >
                      {option.label}
                    </button>
                  );
            ...
            <div className="rounded-3xl border border-white/10 bg-slate-950/60 p-5 md:p-6">
              ...
              <div className="mt-7 rounded-2xl border border-fuchsia-300/15 bg-fuchsia-300/[0.04] p-4">
                <p className="text-xs font-black uppercase tracking-[0.18em] text-fuchsia-200">Why we’re asking</p>
                <p className="mt-2 text-sm leading-6 text-slate-400">Some polls are just for the argument. Others help SingHUB learn what singers actually value so we build the useful stuff first.</p>
              </div>
            </div>
          </div>
        ) : (
          <div className="mt-6 rounded-3xl border border-white/10 bg-black/20 p-6 text-slate-300">{message || "No poll loaded."}</div>
        )}
      </div>
    </section>
  );
