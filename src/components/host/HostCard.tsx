...
function getHostEnergy(host: HostProfile) {
  ...
  return "Mic scene regular";
}

function getNextGigLine(host: HostProfile) {
  const firstGig = Object.entries(host.schedule)
    .flatMap(([day, gigs]) => gigs.map((gig) => ({ day, ...gig })))
    .find(Boolean);
  ...
  return [firstGig.day, firstGig.venueName, firstGig.time].filter(Boolean).join(" / ");
}
...
export function TonightHostCard({ host, gig }: { host: HostProfile; gig: HostGig }) {
  return (
    ...
    >
      ...
      <div className="absolute -right-8 -top-10 h-32 w-32 rounded-full border border-cyan-300/20 bg-cyan-300/10 blur-2xl" />

      <div className="relative z-10 flex w-full flex-col justify-between">
        <div className="flex items-start justify-between gap-3">
          <HostAvatar host={host} />
          <span className="rounded-full border border-red-300/50 bg-red-400/15 px-3 py-1 text-xs font-black uppercase tracking-[0.14em] text-red-100 shadow-lg shadow-red-950/30">
            Tonight
          </span>
        </div>
        ...
        <div>
          ...
          <p className="mt-2 text-sm font-black text-fuchsia-100">{gig.venueName || "Venue TBA"}</p>
    ...
    >
      ...
      <div className="relative z-10">
        ...
        <div className="mt-5 flex flex-wrap gap-2">
          ...
          {host.profileCompletionLevel === "enhanced" && (
            <Badge variant="verified">Verified Host</Badge>
          )}
        </div>

        {host.bio && <p className="mt-4 line-clamp-3 text-sm leading-6 text-slate-200">{host.bio}</p>}
