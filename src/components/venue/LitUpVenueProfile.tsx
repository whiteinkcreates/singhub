...
function groupSpecials(specials: VenueSpecial[]) {
  ...
  for (const special of specials) {
    groups.set(special.day, [...(groups.get(special.day) || []), special]);
  }
...
function ActionLink({ href, children, featured = false }: { href: string; children: string; featured?: boolean }) {
  return (
    <a
      ...
      target="_blank"
      rel="noreferrer"
      className={`inline-flex min-h-12 items-center justify-center rounded-full border px-5 py-3 text-sm font-black transition focus:outline-none focus:ring-2 focus:ring-fuchsia-400 ${
        featured
          ? "border-[#ff2aa3] bg-[#ff2aa3] text-white hover:bg-[#ff4bb2]"
          : "border-white/15 bg-white/[0.04] text-white hover:border-cyan-300/50 hover:bg-cyan-300/[0.06]"
      }`}
    >
      {children}
    </a>
...
export function LitUpVenueProfile({ venue, events = [] }: { venue: VenueListing; events?: KaraokeEventListing[] }) {
  ...
  const todayEvents = events.filter(
    (event) => clean(event.karaokeDay)?.toLowerCase() === weekday.toLowerCase(),
  );
      ...
      <section className="relative min-h-[24rem] overflow-hidden md:min-h-[32rem]">
        ...
        <div className="relative flex min-h-[24rem] flex-col justify-end p-5 md:min-h-[32rem] md:p-8">
          <div className="flex flex-wrap gap-2">
            <span className="rounded-full bg-[#ff2aa3] px-3 py-1.5 text-[0.7rem] font-black uppercase tracking-[0.12em] text-white">
              Karaoke
            </span>
            <span className="rounded-full border border-cyan-300/50 bg-black/45 px-3 py-1.5 text-[0.7rem] font-black uppercase tracking-[0.12em] text-cyan-100 backdrop-blur">
              Lit Up Venue
            </span>
          </div>
          <h1 className="mt-4 text-4xl font-black leading-none text-white drop-shadow-2xl md:text-6xl">
            {venue.venueName}
          </h1>
          <p className="mt-3 text-sm font-semibold text-slate-200 md:text-base">
            {[venue.neighborhood || venue.city, clean(venue.address)].filter(Boolean).join(" • ")}
          </p>
          {(enhancement?.tagline || venue.description) && (
      ...
      <div className="p-4 sm:p-6 md:p-8">
        ...
        <section id="tonight" className="mt-7 rounded-[1.5rem] border border-white/10 bg-white/[0.035] p-5">
          <div className="flex flex-wrap items-end justify-between gap-2">
            ...
            {todayEvents.length === 0 && tonightSpecials.length === 0 && (
              <p className="text-sm text-slate-500">No featured item loaded for tonight yet.</p>
            )}
          </div>

          <div className="mt-5 grid gap-3 md:grid-cols-2">
            {todayEvents.map((event) => (
              <div key={event.eventId} className="rounded-2xl border border-fuchsia-300/20 bg-fuchsia-300/[0.06] p-4">
                <p className="text-xs font-black uppercase tracking-[0.16em] text-fuchsia-200">Karaoke</p>
                <p className="mt-2 text-base font-black text-white">{formatSchedule(event)}</p>
                {clean(event.hostName) && <p className="mt-1 text-sm text-slate-300">KJ: {event.hostName}</p>}
              </div>
            ))}
            {tonightSpecials.map((special) => (
              <div key={`${special.day}-${special.title}`} className="rounded-2xl border border-cyan-300/15 bg-cyan-300/[0.05] p-4">
                ...
                {special.detail && <p className="mt-1 text-sm text-slate-300">{special.detail}</p>}
              </div>
            ))}
          </div>
        </section>
        ...
        {specialGroups.length > 0 && (
          <section id="specials" className="mt-9">
            <div className="flex items-end justify-between gap-4">
              <div>
                <p className="text-xs font-black uppercase tracking-[0.2em] text-fuchsia-300">More reasons to go</p>
                <h2 className="mt-1 text-2xl font-black text-white">Weekly Specials</h2>
              </div>
            </div>
            <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
              {specialGroups.map(([day, specials]) => (
                <div key={day} className="rounded-2xl border border-white/10 bg-white/[0.035] p-4">
                  <p className="text-xs font-black uppercase tracking-[0.18em] text-cyan-200">{day}</p>
                  <div className="mt-3 space-y-3">
                    {specials.map((special) => (
                      <div key={`${day}-${special.title}`}>
                        ...
                        {special.detail && <p className="mt-1 text-xs leading-5 text-slate-400">{special.detail}</p>}
                      </div>
                    ))}
                  </div>
                </div>
        ...
        {Boolean(enhancement?.dailyDeals.length) && (
          <section className="mt-9">
            <p className="text-xs font-black uppercase tracking-[0.2em] text-cyan-300">Every day</p>
            <h2 className="mt-1 text-2xl font-black text-white">Daily Deals</h2>
            <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
              {enhancement?.dailyDeals.map((deal) => (
                <div key={deal.title} className="rounded-2xl border border-white/10 bg-white/[0.035] p-4">
                  <p className="text-lg font-black text-white">{deal.price || ""}</p>
                  <p className="mt-1 text-sm font-bold text-fuchsia-100">{deal.title}</p>
                  {deal.detail && <p className="mt-2 text-xs leading-5 text-slate-400">{deal.detail}</p>}
                </div>
              ))}
            </div>
          </section>
        ...
        <section id="about" className="mt-9 grid gap-6 lg:grid-cols-[1.3fr_0.7fr]">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.2em] text-fuchsia-300">About</p>
            <h2 className="mt-1 text-2xl font-black text-white">Why this place</h2>
            <p className="mt-4 max-w-3xl text-base leading-7 text-slate-300">
              {enhancement?.about || venue.description}
            </p>
          </div>
          {Boolean(enhancement?.amenities.length) && (
            <div className="rounded-[1.5rem] border border-white/10 bg-white/[0.035] p-5">
              <p className="text-xs font-black uppercase tracking-[0.18em] text-cyan-200">Good to know</p>
              <div className="mt-4 flex flex-wrap gap-2">
                {enhancement?.amenities.map((amenity) => (
                  <span key={amenity} className="rounded-full border border-white/10 bg-black/15 px-3 py-2 text-xs font-semibold text-slate-200">
                    {amenity}
                  </span>
                ))}
              </div>
        ...
        {hasGallery && (
          <section id="photos" className="mt-9">
            <p className="text-xs font-black uppercase tracking-[0.2em] text-fuchsia-300">Photos</p>
            <h2 className="mt-1 text-2xl font-black text-white">Inside {venue.venueName}</h2>
            <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {enhancement?.gallery.map((photo) => (
                <figure key={photo.url} className="overflow-hidden rounded-2xl border border-white/10 bg-black/20">
                  <img src={photo.url} alt={photo.alt} className="aspect-[4/3] h-full w-full object-cover" loading="lazy" />
                  {photo.caption && <figcaption className="p-3 text-xs text-slate-400">{photo.caption}</figcaption>}
                </figure>
              ))}
            </div>
          </section>
        ...
        <section id="events" className="mt-9">
