...
export function LitUpVenueTabs({ venueName, enhancement, events, singersSay }: { venueName: string; enhancement: VenueEnhancement; events: KaraokeEventListing[]; singersSay?: SingersSaySummary }) {
  ...
  return (
    <section className="mt-7">
      <div className="flex gap-1 overflow-x-auto border-b border-white/10">
        {tabs.map((item) => <button key={item.id} type="button" onClick={() => setTab(item.id)} className={`whitespace-nowrap border-b-2 px-4 py-3 text-sm font-black transition ${tab === item.id ? "border-[#ff2aa3] text-fuchsia-300" : "border-transparent text-slate-500 hover:text-white"}`}>{item.label}</button>)}
      </div>

      <div className="pt-5">
        {tab === "specials" && (
          <div className="space-y-6">
            {specialGroups.length > 0 && <div><div className="flex items-end justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[0.18em] text-fuchsia-300">Weekly specials</p><h2 className="mt-1 text-2xl font-black text-white">Worth showing up hungry</h2></div></div><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{specialGroups.map(([day, specials]) => <div key={day} className="rounded-2xl border border-white/10 bg-white/[0.035] p-4"><p className="text-xs font-black uppercase tracking-[0.16em] text-cyan-200">{day}</p><div className="mt-3 space-y-3">{specials.map((special) => <div key={`${day}-${special.title}`}><p className="font-black text-white">{special.price ? `${special.price} ` : ""}{special.title}</p>{special.detail && <p className="mt-1 text-xs leading-5 text-slate-400">{special.detail}</p>}</div>)}</div></div>)}</div></div>}
            {enhancement.dailyDeals.length > 0 && <div><p className="text-xs font-black uppercase tracking-[0.18em] text-cyan-300">Every day</p><div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{enhancement.dailyDeals.map((deal) => <div key={deal.title} className="rounded-2xl border border-white/10 bg-white/[0.035] p-4"><p className="text-lg font-black text-white">{deal.price || ""}</p><p className="mt-1 font-bold text-fuchsia-100">{deal.title}</p>{deal.detail && <p className="mt-2 text-sm text-slate-400">{deal.detail}</p>}</div>)}</div></div>}
          </div>
        )}
