...
type PostType = "image" | "note" | "wanted";
...
export function SingBoard({initialFlyers}:{initialFlyers:BoardPost[]}){
  ...
  function createDraft(nextType:PostType,imageUrl?:string){const defaultTitle=nextType==="wanted"?"Join the crew":"Karaoke event";setSelectedSlotId(null);setDraft({id:`draft-${Date.now()}`,postType:nextType,title:title||defaultTitle,venue:venue||"Your venue",neighborhood:neighborhood||"Your neighborhood",region:regionValue(),detail,x:0,y:0,rotation:0,imageUrl,noteText:nextType==="note"||nextType==="wanted"?noteText:undefined,noteColor:nextType==="note"?noteColor:undefined,pinned:false,eventDate:date,startTime:nextType==="wanted"?undefined:time||undefined,hostName:nextType==="wanted"?undefined:host||undefined,linkUrl:link||undefined});}
  ...
  async function copyLastFlyerLink(){if(!lastPublished)return;const target=lastPublished.postType==="wanted"&&lastPublished.linkUrl?new URL(lastPublished.linkUrl,window.location.origin).toString():`${window.location.origin}/events/${lastPublished.id}`;await navigator.clipboard.writeText(target);setBoardExportMessage(`Copied the direct link for ${lastPublished.title}.`);}

  async function publish(){
    if(!draft)return setStatus(type==="image"?"Upload an image first.":type==="wanted"?"Create your wanted poster first.":"Create your note first.");
    ...
    if((type==="note"||type==="wanted")&&!noteText.trim())return setStatus(type==="wanted"?"Add the roles or people you are looking for.":"Write something on the note.");
    setPosting(true);
    ...
    try{
      ...
      form.append("title",title.trim()||(type==="wanted"?"Join the crew":"Karaoke event"));
      ...
      else if(type==="note"){form.append("noteText",noteText.trim());form.append("noteColor",noteColor);}
      else form.append("noteText",noteText.trim());
      ...
      setPosts(p=>[...p,{...draft,id:result.id!,title:title.trim()||(type==="wanted"?"Join the crew":"Karaoke event"),venue:venue.trim(),neighborhood:neighborhood.trim(),detail:detail.trim(),eventDate:date,startTime:type==="wanted"?undefined:time||undefined,hostName:type==="wanted"?undefined:host||undefined,linkUrl:link||undefined,imageUrl:result.imageUrl||draft.imageUrl,noteText:type==="note"||type==="wanted"?noteText.trim():undefined,noteColor:type==="note"?noteColor:undefined,pinned:true}]);
      setLastPublished({id:result.id,title:title.trim()||(type==="wanted"?"Join the crew":"Karaoke event"),postType:type,linkUrl:link||undefined});
    ...
    <div className="grid gap-4 lg:grid-cols-[1fr_360px]">
      ...
      <aside className="space-y-3 rounded-2xl border border-fuchsia-400/15 bg-[linear-gradient(180deg,rgba(15,17,28,.98),rgba(5,7,12,.98))] p-4 text-white shadow-[0_20px_50px_rgba(0,0,0,.35)]">
        ...
        <div className="grid grid-cols-3 gap-2">
          <button onClick={()=>{setType("image");setDraft(null);setSelectedSlotId(null);}} className={`rounded-xl border p-3 text-xs font-black transition sm:text-sm ${type==="image"?"border-fuchsia-300 bg-fuchsia-300/15 shadow-[0_0_16px_rgba(236,72,153,.12)]":"border-white/10 hover:border-white/20"}`}>Image</button>
          <button onClick={()=>{setType("note");fileRef.current=null;createDraft("note");}} className={`rounded-xl border p-3 text-xs font-black transition sm:text-sm ${type==="note"?"border-violet-300 bg-violet-300/15 shadow-[0_0_16px_rgba(139,92,246,.12)]":"border-white/10 hover:border-white/20"}`}>Note</button>
          <button onClick={()=>{setType("wanted");fileRef.current=null;createDraft("wanted");}} className={`rounded-xl border p-3 text-xs font-black transition sm:text-sm ${type==="wanted"?"border-orange-300 bg-orange-300/15 shadow-[0_0_16px_rgba(251,146,60,.12)]":"border-white/10 hover:border-white/20"}`}>Wanted</button>
        </div>
        {type==="image"
          ...
          :type==="note"
            ...
            :<textarea value={noteText} maxLength={220} onChange={e=>{setNoteText(e.target.value);updateDraft({noteText:e.target.value});}} placeholder={"Roles wanted\nBartenders • Servers • Kitchen Help"} className="min-h-28 w-full rounded-xl border border-orange-300/35 bg-white/[.06] p-3 outline-none focus:border-orange-300"/>}
        <input value={title} onChange={e=>{setTitle(e.target.value);updateDraft({title:e.target.value});}} placeholder={type==="wanted"?"Wanted headline (e.g. Join the crew)":"Headline / event name"} className="w-full rounded-xl border border-white/15 bg-white/[.06] p-3 outline-none focus:border-fuchsia-300"/>
        ...
        <div>
          <p className="mb-1 text-[11px] font-bold uppercase tracking-[.14em] text-slate-500">{type==="wanted"?"Archive / refresh after":"Event date"}</p>
          <div className={type==="wanted"?"grid":"grid grid-cols-2 gap-2"}>
            ...
            {type!=="wanted"&&<input value={time} onChange={e=>{setTime(e.target.value);updateDraft({startTime:e.target.value});}} placeholder="Start time" className="rounded-xl border border-white/15 bg-white/[.06] p-3 outline-none focus:border-cyan-300"/>}
          </div>
        </div>
        {type!=="wanted"&&<input value={host} onChange={e=>{setHost(e.target.value);updateDraft({hostName:e.target.value});}} placeholder="Host / KJ (optional)" className="w-full rounded-xl border border-white/15 bg-white/[.06] p-3 outline-none focus:border-violet-300"/>}
        <textarea value={detail} maxLength={1600} onChange={e=>{setDetail(e.target.value);updateDraft({detail:e.target.value});}} placeholder={type==="wanted"?"Full job ad copy / candidate pitch":"Event details for the SingHUB page (optional)"} className="min-h-24 w-full rounded-xl border border-white/15 bg-white/[.06] p-3 outline-none focus:border-violet-300"/>
        <input value={link} onChange={e=>{setLink(e.target.value);updateDraft({linkUrl:e.target.value});}} placeholder={type==="wanted"?"Full job ad URL (e.g. /jobs/barlando)":"Official event / ticket link (optional)"} className="w-full rounded-xl border border-white/15 bg-white/[.06] p-3 outline-none focus:border-cyan-300"/>
