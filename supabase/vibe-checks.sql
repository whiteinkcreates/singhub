...
comment on table public.feature_poll_votes is
  'Legacy authenticated roadmap votes retained for migration and audit history.';

-- Roadmap voting is intentionally lower-friction than venue Vibe Checks.
-- The vote is advisory, so an email is enough to deduplicate and count it.
-- Venue reports still require a confirmed Supabase Auth identity.

create table if not exists public.feature_poll_email_votes (
  id uuid primary key default gen_random_uuid(),
  email text not null,
  poll_slug text not null,
  option_id text not null,
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint feature_poll_email_option_reference foreign key (poll_slug, option_id)
    references public.feature_poll_options(poll_slug, option_id),
  constraint feature_poll_one_vote_per_email unique (email, poll_slug)
);

create index if not exists feature_poll_email_results_lookup
  on public.feature_poll_email_votes (poll_slug, option_id);

alter table public.feature_poll_email_votes enable row level security;

-- Preserve any votes already cast through the original confirmed-member flow.
insert into public.feature_poll_email_votes (
  email,
  poll_slug,
  option_id,
  product_updates_opt_in,
...
select
  ...
  votes.poll_slug,
  votes.option_id,
  profiles.product_updates_opt_in,
  profiles.updates_opted_in_at,
  votes.created_at,
  votes.updated_at
from public.feature_poll_votes votes
join public.singhub_member_profiles profiles on profiles.user_id = votes.user_id
on conflict (email, poll_slug) do nothing;

create or replace function public.submit_feature_email_vote(
  p_email text,
  p_poll_slug text,
  p_option_id text,
  p_product_updates_opt_in boolean default false
...
begin
  ...
  on conflict (email, poll_slug) do update set
    option_id = excluded.option_id,
    product_updates_opt_in = excluded.product_updates_opt_in,
    updates_opted_in_at = case
      when excluded.product_updates_opt_in then coalesce(
        public.feature_poll_email_votes.updates_opted_in_at,
        now()
      )
      else null
    end,
...
revoke all on function public.submit_feature_email_vote(text, text, text, boolean) from public;
grant execute on function public.submit_feature_email_vote(text, text, text, boolean) to service_role;

comment on table public.feature_poll_email_votes is
  'Private, deduplicated roadmap votes. Emails are never returned by public routes.';
